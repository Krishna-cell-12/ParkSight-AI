...
for p in ep['priority_list'][:5]:
    ...
    enforcement_priority.append({
        ...
        'locationType': p.get('locationType', 'junction'),
    })
...
# ── 12b. NEXT-DAY FORECAST ────────────────────────────────────
with open('parksight_output/nextday_forecast.json') as f:
    nextday_raw = json.load(f)

with open('parksight_output/model_logic.json') as f:
    model_logic_data = json.load(f)

# Convert snake_case JSON keys to camelCase for the dashboard
def snake_to_camel_junction(jdata):
    """Convert a junction dict from snake_case to camelCase."""
    return {
        'shortName':        jdata.get('shortName', jdata.get('short_name', '')),
        'color':            jdata.get('color', '#6B7280'),
        'avgDaily':         jdata.get('avgDaily', jdata.get('avg_daily', 0)),
        'highThreshold':    jdata.get('highThreshold', jdata.get('high_threshold', 0)),
        'mediumThreshold':  jdata.get('mediumThreshold', jdata.get('medium_threshold', 0)),
        'modelUsed':        jdata.get('modelUsed', jdata.get('model_used', 'Unknown')),
        'testMAE':          jdata.get('testMAE', jdata.get('test_mae', 0)),
        'testRMSE':         jdata.get('testRMSE', jdata.get('test_rmse', 0)),
        'testR2':           jdata.get('testR2', jdata.get('test_r2', 0)),
        'accuracy':         jdata.get('accuracy', 0),
        'featureImportance': jdata.get('featureImportance', jdata.get('feature_importance', {})),
        'days':             jdata.get('days', []),
    }

def snake_to_camel_accuracy(adata):
    return {
        'shortName': adata.get('shortName', adata.get('short_name', '')),
        'model':     adata.get('model', 'Unknown'),
        'mae':       adata.get('mae', 0),
        'rmse':      adata.get('rmse', 0),
        'r2':        adata.get('r2', 0),
        'accuracy':  adata.get('accuracy', 0),
        'nDays':     adata.get('nDays', adata.get('n_days', 0)),
    }

nextday_forecast_export = {
    'generatedAt':    nextday_raw.get('generated_at', nextday_raw.get('generatedAt', '')),
    'dataDateRange':  nextday_raw.get('data_date_range', nextday_raw.get('dataDateRange', '')),
    'forecastFrom':   nextday_raw.get('forecast_from', nextday_raw.get('forecastFrom', '')),
    'forecastTo':     nextday_raw.get('forecast_to', nextday_raw.get('forecastTo', '')),
    'modelsUsed':     nextday_raw.get('models_used', nextday_raw.get('modelsUsed', [])),
    'featuresUsed':   nextday_raw.get('features_used', nextday_raw.get('featuresUsed', [])),
    'cityWide':       nextday_raw.get('city_wide', nextday_raw.get('cityWide', {})),
    'junctions':      {k: snake_to_camel_junction(v)
                       for k, v in nextday_raw.get('junctions', {}).items()},
    'modelAccuracySummary': {k: snake_to_camel_accuracy(v)
                             for k, v in nextday_raw.get(
                                 'model_accuracy_summary',
                                 nextday_raw.get('modelAccuracySummary', {})).items()},
}
...
lines = [
    ...
    '// Next-day forecast — generated by phase3_nextday_model.py',
    f"export const NEXTDAY_FORECAST = {js_val(nextday_forecast_export)};",
    f"export const MODEL_LOGIC = {js_val(model_logic_data)};",
]
